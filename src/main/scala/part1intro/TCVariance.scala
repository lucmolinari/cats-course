...
object TCVariance {
  ...
  // val anInvalidComparison = Some(2) === None //Eq[Some[Int]] not found - doesn't compile
  ...
  // but may confuse the compiler if the general TC is also available
  ...
  def main(args: Array[String]): Unit = {
    ...
    // println(organizeShow[Animal]) // will not compile - 2 potential instances - ambiguous values
  }
